...
uses Classes, CastleWindow,
  ...
  CastleCreatures, ThirdPersonJumper, Carrot, GameStateMain, GameStateLevelSwim;

type
  { Main "playing game" state, where most of the game logic takes place. }
  TStateLevelCave = class(TUIState)
  private
...
var
  StateLevelCave: TStateLevelCave;
...
constructor TStateLevelCave.Create(AOwner: TComponent);
begin
  inherited;
  DesignUrl := 'castle-data:/level_cave/gamestatelevelcave.castle-user-interface';
end;

procedure TStateLevelCave.Start;
var
...
procedure TStateLevelCave.Stop;
begin
  inherited;
end;

procedure TStateLevelCave.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    A: TCastleTransform;
    Gem: TCastleSphere;
    I: Integer;
...
begin
  ...
  { Let them go to next level after getting 10 gems }
  J := 0;
  GemsToGo := 10;
  ...
  if GemsToGo <= 0 then
  begin
    LevelNext := TStateLevelSwim.Create(Application);
...
function TStateLevelCave.Press(const Event: TInputPressRelease): Boolean;
...
begin
  ...
  { This virtual method is executed when user presses
    ...
    The TStateLevelCave.Press method should be used to handle keys
...
procedure TStateLevelCave.ChangeCheckboxCameraFollows(Sender: TObject);
begin
  ThirdPersonNavigation.CameraFollows := CheckboxCameraFollows.Checked;
end;

procedure TStateLevelCave.ChangeCheckboxAimAvatar(Sender: TObject);
begin
...
procedure TStateLevelCave.ChangeCheckboxDebugAvatarColliders(Sender: TObject);
begin
  DebugAvatar.Exists := CheckboxDebugAvatarColliders.Checked;
end;

procedure TStateLevelCave.ChangeCheckboxImmediatelyFixBlockedCamera(Sender: TObject);
begin
