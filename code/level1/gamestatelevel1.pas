...
uses Classes, CastleWindow,
  ...
  CastleCreatures, ThirdPersonJumper, Carrot, GameStateMain, GameStateLevelCave;

type
  { Main "playing game" state, where most of the game logic takes place. }
  TStateLevel1 = class(TUIState)
  private
...
constructor TStateLevel1.Create(AOwner: TComponent);
begin
  inherited;
  DesignUrl := 'castle-data:/level1/gamestatelevel1.castle-user-interface';
end;

procedure TStateLevel1.Start;
var
...
procedure TStateLevel1.Stop;
begin
  inherited;
end;

procedure TStateLevel1.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    A: TCastleTransform;
    Gem: TCastleSphere;
    I: Integer;
...
begin
  ...
  if J = 0 then
  begin
    LevelCave := TStateLevelCave.Create(Application);
    TUIState.Current := LevelCave;
    Exit;

  end;
  LabelGems.Caption := 'Num Gems Remaining: ' + J.ToString;

  if SceneAvatar.Translation.Y < -20 then
  begin
...
function TStateLevel1.Press(const Event: TInputPressRelease): Boolean;
...
begin
  ...
  { This virtual method is executed when user presses
    ...
    The TStateLevel1.Press method should be used to handle keys
...
procedure TStateLevel1.ChangeCheckboxCameraFollows(Sender: TObject);
begin
  ThirdPersonNavigation.CameraFollows := CheckboxCameraFollows.Checked;
end;

procedure TStateLevel1.ChangeCheckboxAimAvatar(Sender: TObject);
begin
...
procedure TStateLevel1.ChangeCheckboxDebugAvatarColliders(Sender: TObject);
begin
  DebugAvatar.Exists := CheckboxDebugAvatarColliders.Checked;
end;

procedure TStateLevel1.ChangeCheckboxImmediatelyFixBlockedCamera(Sender: TObject);
begin
