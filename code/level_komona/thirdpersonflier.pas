...
type
  TThirdPersonFlier = class(TCastleThirdPersonNavigation)
  private
...
constructor TThirdPersonFlier.Create(AOwner: TComponent);
begin
  inherited;
...
{ PreferredHeight := -1.0; }
  DistanceToAvatarTarget := 6.0;
  MinDistanceToAvatarTarget := 3.0;
  AvatarTarget := Vector3(0, 0, 0);
...
procedure TThirdPersonFlier.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    B: TCastleTransform;
...
function TThirdPersonFlier.Jump: boolean;
begin
  Result := false;
...
function TThirdPersonFlier.MaxJumpDistance: Single;
begin
  Result := JumpMaxHeight;
end;

function TThirdPersonFlier.Press(const Event: TInputPressRelease): boolean;
    var
      B: TCastleTransform;
begin
  inherited;
