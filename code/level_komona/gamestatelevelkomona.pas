...
{ Level Flying in Komona logic: }
unit GameStateLevelKomona;
...
var
  StateLevelKomona: TStateLevelKomona;

implementation

uses SysUtils, Math, StrUtils,
  CastleSoundEngine, CastleLog, CastleStringUtils, CastleFilesUtils,
  CastleUtils, GameStateMain;
 

{ TStateLevelKomona ----------------------------------------------------------------- }

constructor TStateLevelKomona.Create(AOwner: TComponent);
begin
  inherited;
  DesignUrl := 'castle-data:/level_komona/gamestatelevelkomona.castle-user-interface';
end;

procedure TStateLevelKomona.Start;
var
...
begin
  ...
  ThirdPersonNavigation := TThirdPersonFlier.Create(MainViewport);
  ...
  ThirdPersonNavigation.MoveSpeed := 10.0;
...
procedure TStateLevelKomona.Stop;
begin
  inherited;
end;

procedure TStateLevelKomona.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    Gem: TCastleSphere;
    I: Integer;
    J: Integer;
...
begin
  ...
  { Let them go to next level after getting 8 gems }
  J := 0;
  GemsToGo := 8;
  ...
  if GemsToGo <= 0 then
  begin
    LevelNext := TStateRivers.Create(Application);
...
function TStateLevelKomona.Press(const Event: TInputPressRelease): Boolean;
...
begin
  ...
  { This virtual method is executed when user presses
    ...
    The TStateLevelKomona.Press method should be used to handle keys
...
procedure TStateLevelKomona.ChangeCheckboxCameraFollows(Sender: TObject);
begin
  ThirdPersonNavigation.CameraFollows := CheckboxCameraFollows.Checked;
end;

procedure TStateLevelKomona.ChangeCheckboxAimAvatar(Sender: TObject);
begin
...
procedure TStateLevelKomona.ChangeCheckboxDebugAvatarColliders(Sender: TObject);
begin
  DebugAvatar.Exists := CheckboxDebugAvatarColliders.Checked;
end;

procedure TStateLevelKomona.ChangeCheckboxImmediatelyFixBlockedCamera(Sender: TObject);
begin
