...
{ Level Flying in Clouds logic: }
unit GameStateLevelClouds;
...
uses Classes, CastleWindow,
  ...
  ThirdPersonFlier, GameStateLevelSnowball;

type
  TStateLevelClouds = class(TUIState)
  private
...
var
  StateLevelClouds: TStateLevelClouds;
...
constructor TStateLevelClouds.Create(AOwner: TComponent);
begin
  inherited;
  DesignUrl := 'castle-data:/level_clouds/gamestatelevelclouds.castle-user-interface';
end;

procedure TStateLevelClouds.Start;
var
  I: Integer;
begin
  inherited;
  ...
  ThirdPersonNavigation.MoveSpeed := 8.0;
...
procedure TStateLevelClouds.Stop;
begin
  inherited;
end;

procedure TStateLevelClouds.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    Gem: TCastleSphere;
    I: Integer;
    J: Integer;
...
begin
  ...
  if GemsToGo <= 0 then
  begin
    LevelNext := TStateLevelSnowball.Create(Application);
...
function TStateLevelClouds.Press(const Event: TInputPressRelease): Boolean;
...
begin
  ...
  { This virtual method is executed when user presses
    ...
    The TStateLevelClouds.Press method should be used to handle keys
...
procedure TStateLevelClouds.ChangeCheckboxCameraFollows(Sender: TObject);
begin
  ThirdPersonNavigation.CameraFollows := CheckboxCameraFollows.Checked;
end;

procedure TStateLevelClouds.ChangeCheckboxAimAvatar(Sender: TObject);
begin
...
procedure TStateLevelClouds.ChangeCheckboxDebugAvatarColliders(Sender: TObject);
begin
  DebugAvatar.Exists := CheckboxDebugAvatarColliders.Checked;
end;

procedure TStateLevelClouds.ChangeCheckboxImmediatelyFixBlockedCamera(Sender: TObject);
begin
