...
uses Classes, CastleWindow,
  ...
  ThirdPersonSnowball, Snowball, GameStateMain, GameStateLevelSnowman;

type
  TStateLevelSnowball3 = class(TUIState)
  private
...
{ DebugSnowballs: TDebugTransform; }
    LevelSnowman: TStateLevelSnowman;
...
var
  StateLevelSnowball3: TStateLevelSnowball3;
...
{ TStateLevelSnowball3 ----------------------------------------------------------------- }

constructor TStateLevelSnowball3.Create(AOwner: TComponent);
begin
  inherited;
  DesignUrl := 'castle-data:/level_snowball/gamestatelevelsnowball3.castle-user-interface';
...
procedure TStateLevelSnowball3.Start;
var
...
procedure TStateLevelSnowball3.Stop;
begin
{ FreeAndNil(Enemies); }
  inherited;
end;

procedure TStateLevelSnowball3.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    Enemy: TCastleScene;
    I: Integer;
    J: Integer;
...
begin
  ...
  if J = 0 then
  begin
    LevelSnowman := TStateLevelSnowman.Create(Application);
    TUIState.Current := LevelSnowman;
    Exit;
...
function TStateLevelSnowball3.Press(const Event: TInputPressRelease): Boolean;
...
begin
  ...
  { This virtual method is executed when user presses
    ...
    The TStateLevelSnowball3.Press method should be used to handle keys
...
procedure TStateLevelSnowball3.ChangeCheckboxCameraFollows(Sender: TObject);
begin
  ThirdPersonNavigation.CameraFollows := CheckboxCameraFollows.Checked;
end;

procedure TStateLevelSnowball3.ChangeCheckboxAimAvatar(Sender: TObject);
begin
...
procedure TStateLevelSnowball3.ChangeCheckboxDebugAvatarColliders(Sender: TObject);
begin
  DebugAvatar.Exists := CheckboxDebugAvatarColliders.Checked;
end;

procedure TStateLevelSnowball3.ChangeCheckboxImmediatelyFixBlockedCamera(Sender: TObject);
begin
  ThirdPersonNavigation.ImmediatelyFixBlockedCamera := CheckboxImmediatelyFixBlockedCamera.Checked;
end;

procedure TStateLevelSnowball3.SpawnSnowball;
var
  Snowball: TSnowball;
begin
