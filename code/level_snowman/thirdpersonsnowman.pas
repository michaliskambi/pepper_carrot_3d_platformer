...
CastleLog, CastleVectors, CastleScene;

type
  TThirdPersonSnowman = class(TCastleThirdPersonNavigation)
  private
...
constructor TThirdPersonSnowman.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FSnowballGrowSpeed := 0.01;
  { LastPosition := Avatar.Translation; }
end;

procedure TThirdPersonSnowman.Update(const SecondsPassed: Single; var HandleInput: Boolean);
  var
    B: TCastleTransform;
    SnowballGrow: Single;
...
begin
  ...
  Movement := B.Translation - LastPosition;
  FMovement := sqr((Movement.x * Movement.x) +
                   (Movement.z * Movement.z));
  { WriteLnLog(FloatToStr(FMovement)); }
  
  { Snowball := B.FindRequiredComponent('Snowball') as TCastleSphere; }
  SnowballGrow := FMovement * FSnowballGrowSpeed;
  Snowball.Radius := Snowball.Radius + SnowballGrow;
  Snowball.Translation := Vector3(
    Snowball.Translation.X,
    Snowball.Translation.Y + (SnowballGrow / 6),
    Snowball.Translation.Z + (SnowballGrow / 6)
  );
  MaxDistanceToAvatarTarget := MaxDistanceToAvatarTarget + (SnowballGrow / 6);
end;

function TThirdPersonSnowman.Jump: boolean;
begin
  Result := false;
...
function TThirdPersonSnowman.MaxJumpDistance: Single;
begin
  Result := JumpMaxHeight;
end;

function TThirdPersonSnowman.Press(const Event: TInputPressRelease): boolean;
begin
  inherited;
